...
function App() {
	...
	useEffect(() => {
		const storedWrongAnswers = JSON.parse(sessionStorage.getItem('wrongAnswers'));
	...
	const handleAnswerClick = (isCorrect) => {
		...
		if (nextIndex < questions.length) {
			...
			// Reset the questions after 40 questions
			if (score + wrongAnswers.length >= 504) {
				setCurrentIndex(0);
			}
		}
	};
	...
	const handleResetQuiz = () => {
		...
		setShowPopup(false); // Close the popup when resetting the quiz
	};
  
	const handleReanswerClick = () => {
	  // When the reanswer button is clicked, set the popup index to the first wrong question.
	...
	const handleNextPopup = () => {
	  if (popupIndex < wrongAnswers.length - 1) {
	...
	const handleSkipToQuestion = () => {
	  if (skipToQuestion >= 1 && skipToQuestion <= questions.length) {
		setCurrentIndex(skipToQuestion - 1);
		setShowPopup(false); // Close the popup when skipping
	  }
	};
...
function Question({ question, handleAnswerClick }) {
	return (
		<div className='quiz-context justify-center align-center flex-column'>
			<p className='question'>{question.question}</p>
			{question.answers.map((answer, index) => {
				const { text, correct } = answer;
				return (
					<button key={index} onClick={() => handleAnswerClick(correct)}>
					{text}
					</button>
				);
			})}
